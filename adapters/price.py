...
from tw_stock_ai.adapters.base import AdapterFetchRequest, AdapterFetchResult, PriceDataAdapter
...
from tw_stock_ai.adapters.unavailable import UnavailablePriceAdapter
from tw_stock_ai.config import Settings, get_settings


class FugleHistoricalPriceAdapter(PriceDataAdapter):
    adapter_name = "fugle_historical_price"
    ...
    def fetch(self, request: AdapterFetchRequest) -> AdapterFetchResult:
        ...
        if not symbols:
            return self.unavailable_result("no_symbols_resolved_for_fugle_price", request=request, detail=";".join(errors))
        ...
        for symbol in symbols:
            ...
            for row in candles:
                ...
                open_price = parse_float(str(row.get("open", "")))
                high_price = parse_float(str(row.get("high", "")))
                low_price = parse_float(str(row.get("low", "")))
                close_price = parse_float(str(row.get("close", "")))
                volume = parse_int(str(row.get("volume", "")))
                if not all(value is not None for value in [trade_date, open_price, high_price, low_price, close_price, volume]):
                    errors.append(f"{symbol}:incomplete_candle")
                    continue
                raw_items.append(
                    {
        ...
        return AdapterFetchResult(
            ...
            detail=None if cleaned_items else "no_price_rows_loaded",
            errors=errors,
...
__all__ = ["PriceDataAdapter", "FugleHistoricalPriceAdapter", "UnavailablePriceAdapter"]
