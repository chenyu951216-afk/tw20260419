...
templates = Jinja2Templates(directory="src/tw_stock_ai/templates")
...
def run_picks() -> RedirectResponse:
    with SessionLocal() as session:
        ...
        try:
            ...
            return _redirect("/picks", flash=f"重新執行選股被限流: {exc}")
...
async def save_settings_page(request: Request) -> RedirectResponse:
    ...
    with SessionLocal() as session:
        try:
            ...
            return _redirect("/settings", flash=f"設定保存失敗: {exc}")

    suffix = "設定已保存。"
    if {"screening_hour", "screening_minute"} & set(changed):
        suffix = "設定已保存。排程時間變更會在 worker 重啟後套用到下一輪 cron。"
