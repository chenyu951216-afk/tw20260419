...
- `docs/zeabur.web.env.example`
  - full environment variable template for Zeabur `web`
- `docs/zeabur.worker.env.example`
  - full environment variable template for Zeabur `worker`
...
If Zeabur does not expand `${POSTGRES_CONNECTION_STRING}` for your service, paste the full PostgreSQL connection string directly into `DATABASE_URL` instead.
...
- worker now runs the real-data refresh pipeline before scheduled screening and Discord push
